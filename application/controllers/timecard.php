...
class Timecard extends CI_Controller {
    ...
    function process_add_timecard()
    {
        //add a new timecard entry for user
		$this->load->library('form_validation');

		//need to validate for each line -
		// - IF totalHours == 0 then dont worry about blank dateInput value and dont enter
		//that line in database


		$this->form_validation->set_rules('totalHours', 'Total Hours', 'required|numeric');
        $this->form_validation->set_rules('dateInput', 'Entry Date', 'required');
